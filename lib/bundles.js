...
function normalizeBundle (bundle = {}, options = {}, index, data = {}) {
  ...
  // Normalize input.
  //
  if (bundle.input instanceof Array) {
  ...
  } else if (typeof bundle.input === 'string') {
    if (bundle.content) {
      bundle.input = [{
        path: bundle.input,
  ...
  } else if (typeof bundle.input === 'object' && bundle.input.constructor === Object) {
    bundle.input = [bundle.input]
  }
  ...
  bundle.input.forEach((filepath, i) => {
    ...
    const file = bundle.output[i] = readFile(filepath, options.frontMatter || {})
    // Merge data.
    file.data = merge(
      {},
      file.data,
