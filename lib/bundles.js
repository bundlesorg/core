        ...
        .on('change', (filepath) => {
          ...
          bundle.output.some((file, i) => {
            if (file.source.path === filepath) {
              bundle.output[i] = Object.assign(bundle.output[i], readFile(filepath, config.options.frontMatter || {}))
              return true
            }
            return false
          })
...
function normalizeBundle (bundle = {}, options = {}, index, data = {}) {
  ...
  // Normalize input.
  //
  if (typeof bundle.input === 'string') {
    bundle._meta.inputIsContent = true
    bundle.input = [bundle.input]
  } else if (bundle.input instanceof Array) {
  ...
  bundle.input.forEach((filepath, i) => {
    ...
    const file = bundle.output[i] = readFile(filepath, options.frontMatter || {}, bundle._meta.inputIsContent)
    // Merge data.
    file.data = merge(
      {},
      file.data,
      typeof data === 'function' ? data(file, bundle) : data || {},
      typeof bundle.data === 'function' ? bundle.data(file, bundle) : bundle.data || {}
    )
  })
...
function readFile (filepath, options = {}, inputIsContent = false) {
  const file = {}
  file.source = inputIsContent ? matter(filepath, options) : matter.read(filepath, options)
  file.source.path = inputIsContent ? undefined : path.normalize(filepath)
