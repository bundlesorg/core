...
function prepResults (result = {}) {
  ...
  if (result.debug && result.errors.length) {
    console.log(result.errors.length > 1 ? `There were ${result.errors.length} errors...` : 'There was an error...')
...
function normalizeBundle (bundle = {}, options = {}, index) {
  ...
  bundle.input.forEach((input, i) => {
    ...
    if (input.indexOf('gh:') === 0) input = `https://github.com/${input.slice(3)}.git`
